...
<p>WARNING: this page is intended to aid development. <a href="?page=home">Return now</a> if you're not sure what this is.</p>
<?php
			...
			if ($_GET["option"] == "delusers") {
				...
				while ($row = Sql_Fetch_Array($req)) {
					...
					if (is_array($tagged) && in_array($row["id"], array_keys($tagged))) {
						...
						$list .= sprintf('<input type=checkbox name="tagged[%d]" value="1">&nbsp;  ', $row["id"]) . PageLink2("user&id=" . $row["id"] . "&returnpage=admindb&returnoption=delusers", "User " . $row["id"]) . "    [" . $row["email"] . ']<br/>';
					}
				}
				if ($deleted)
			...
			if ($_GET["option"] == "delmsghistory") { ///
				...
				while ($row = Sql_Fetch_Array($req)) {
					...
					if (is_array($tagged) && in_array($row["id"], array_keys($tagged))) {
						...
						$deleted++;
            $list .= sprintf('<input type=checkbox name="tagged[%d]" value="1">&nbsp;  ', $row["id"]) . PageLink2("message&id=" . $row["id"] . "&returnpage=admindb&returnoption=delmsghistory", "Message " . $row["id"]) . "    [" . $row["subject"] . ']<br/>';
					} else {
						$list .= sprintf('<input type=checkbox name="tagged[%d]" value="1">&nbsp;  ', $row["id"]) . PageLink2("message&id=" . $row["id"] . "&returnpage=admindb&returnoption=delmsghistory", "Message " . $row["id"]) . "    [" . $row["subject"] . ']<br/>';
					}
				}
...
<p><?php echo PageLink2("admindb&option=delusers",$GLOBALS['I18N']->get("Delete users..."))?>
<p><?php echo PageLink2("admindb&option=delmsghistory",$GLOBALS['I18N']->get("Delete message history..."))?>
<p><?php echo PageLink2("admindb",$GLOBALS['I18N']->get("Return to database manager"))?>
<hr>

<!--
<p><?php echo PageLink2("admindb&option=nolists",$GLOBALS['I18N']->get("Delete all users who are not subscribed to any list"))?>
<p><?php echo PageLink2("admindb&option=adduniqid",$GLOBALS['I18N']->get("Make sure that all users have a UniqID"))?>
<p><?php echo PageLink2("admindb&option=markinvalidunconfirmed",$GLOBALS['I18N']->get("Mark all users with an invalid email as unconfirmed"))?>
<p><?php echo PageLink2("admindb&option=deleteinvalidemail",$GLOBALS['I18N']->get("Delete users who have an invalid email"))?>
<p><?php echo PageLink2("admindb&option=markallhtml",$GLOBALS['I18N']->get("Mark all users to receive HTML"))?>
<p><?php echo PageLink2("admindb&option=markalltext",$GLOBALS['I18N']->get("Mark all users to receive text"))?>
<p><?php echo PageLink2("admindb&option=markallconfirmed",$GLOBALS['I18N']->get("Mark all users confirmed"))?>
<p><?php echo $GLOBALS['I18N']->get('To try to (automatically)')?> <?php echo PageLink2("admindb&option=fixinvalidemail",$GLOBALS['I18N']->get("Fix emails for users who have an invalid email"))?>
<p><?php echo PageLink2("admindb&option=removestaleentries",$GLOBALS['I18N']->get("Remove Stale entries from the database"))?>
<p><?php echo PageLink2("admindb&option=mergeduplicates",$GLOBALS['I18N']->get("Merge Duplicate Users"))?>
<hr>
<form method=get>

<input type=hidden name="page" value="admindb">
