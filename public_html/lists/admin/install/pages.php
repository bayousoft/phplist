...
switch ($actualPage) {
  ...
  case "install01":
    $editable = "";
    if (isset($_SESSION['database_root_user']) && isset($_SESSION['database_root_pass']) && !empty($_SESSION['database_root_user']) && !empty($_SESSION['database_root_pass'])) {
...
//    if ($GLOBALS['adodb_connection']) Sql_Set_Search_Path($_SESSION['database_schema']);
    
    if ($test_connection) {
    $GLOBALS["database_connection"] = $test_connection;
    #include("install/dbchecking.php"); not fully tested yet
    }
